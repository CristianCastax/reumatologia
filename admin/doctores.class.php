<?php
    ...
    class Doctores extends Sistema{
        ...
        function getOne($id_doctor){
            ...
            $stmt = $this->conn->prepare("SELECT id_doctor, segundo_apellido ,primer_apellido, nombre, fotografia 
                                        FROM doctor
            ...
            if (isset($datos[0])) { //si existe el primer elemento, regresame el primer elemento o si no regresame el arreglo vacio
                return $datos[0];
            }
            return array();
        }
        ...
        function delete($id_doctor){
            $this->conection();
            $stmt = $this->conn->prepare("DELETE FROM doctor 
                                        WHERE id_doctor = :id_doctor;");
            $stmt->bindParam(':id_doctor', $id_doctor, PDO::PARAM_INT); //PDO::PARAM_INT es para que sea un entero
            $stmt->execute();
            return $stmt->rowCount(); //rowCount es para saber cuantas filas se afectaron, es 0 o es 1
